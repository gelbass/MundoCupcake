			...
			{cart.cantCompra == 0 ? <div className="container text-center">
				...
				< div className="container">
					<table className="table m-4">
						...
						<tbody>
							{cart.itemCart.map(item => <tr key={item.id}><td><img className="img-thumbnail" src={item.urlImg} alt={item.nombre} /></td><td>{item.nombre}</td><td>{item.itemAdd}</td><td>{item.totalItem}</td><td><input className="btn btn-verde" type="button" value="Eliminar" onClick={() => cart.deleteItem(item.id)} /></td></tr>)}
						</tbody>
						<tfoot>
							<tr>
								<td></td>
